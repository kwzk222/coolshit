...
public class ModMenuIntegration implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parent -> new Screen(Text.literal("Tutorial Mod Settings")) {
            @Override
            protected void init() {
                super.init();
                // Add button for General Settings
                this.addDrawableChild(ButtonWidget.builder(Text.literal("General Settings"), button -> {
                    this.client.setScreen(buildGeneralConfigScreen(this));
                }).dimensions(this.width / 2 - 105, this.height / 6 + 24, 210, 20).build());

                // Add button for Chat Settings
                this.addDrawableChild(ButtonWidget.builder(Text.literal("Chat Settings"), button -> {
                    this.client.setScreen(AutoConfig.getConfigScreen(ChatConfig.class, this).get());
                }).dimensions(this.width / 2 - 105, this.height / 6 + 48, 210, 20).build());
            }
    ...
    private Screen buildGeneralConfigScreen(Screen parent) {
